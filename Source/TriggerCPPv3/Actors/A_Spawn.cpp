...
#include <TriggerCPPv3/UI/A_Score.h>
...
AA_Spawn::AA_Spawn()
{
	...
	SherrifSpawnComp = CreateDefaultSubobject<UChildActorComponent>(TEXT("SherrifSpawn"));
	BanditSpawnComp = CreateDefaultSubobject<UChildActorComponent>(TEXT("BanditSpawn"));
	...
	SherrifSpawnComp->SetupAttachment(Comp);
	BanditSpawnComp->SetupAttachment(Comp);
	...
	static ConstructorHelpers::FObjectFinder<UMaterialInstance> material(TEXT("/Script/Engine.MaterialInstanceConstant'/Game/UI/Materials/MI_Blocker.MI_Blocker'"));
	Blocker->SetStaticMesh(plane.Object);
	Blocker->SetMaterial(0, material.Object);

	SherrifSpawnComp->AddRelativeLocation(FVector(1200, 0, 0));
	SherrifSpawnComp->SetRelativeRotation(FRotator(0, 180, 0));
	BanditSpawnComp->AddRelativeLocation(FVector(-1200, 0, 0));

	Blocker->SetRelativeRotation(FRotator(0.000001, 630.000001, 450.000000));
	Blocker->SetRelativeScale3D(FVector(14));
	...
	Overlapper->SetBoxExtent(FVector(1250, 350, 350));
	Overlapper->AddRelativeLocation(FVector(0, 0, 350));
	Overlapper->OnComponentEndOverlap.AddDynamic(this, &AA_Spawn::OnOverlap);

	SherrifSpawnComp->SetMobility(EComponentMobility::Movable);
	BanditSpawnComp->SetMobility(EComponentMobility::Movable);
	...
	SherrifSpawnComp->SetChildActorClass(APlayerStart::StaticClass());
	BanditSpawnComp->SetChildActorClass(APlayerStart::StaticClass());
}
...
void AA_Spawn::BeginPlay()
{
	...
	SherrifSpawnComp->SetRelativeLocation(FVector(600 / 2, 0, 0));
	BanditSpawnComp->SetRelativeLocation(FVector(-600 / 2, 0, 0));
}
